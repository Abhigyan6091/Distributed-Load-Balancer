"""
Automated Orchestrator for the 4 Distributed Systems:
Sys1 (Server/LB) : 10.1.75.79:2237 (Internal IP: 172.17.0.38)
Sys2 (Backend 1) : 10.1.75.79:2238 (Internal IP: 172.17.0.39)
Sys3 (Backend 2) : 10.1.75.79:2239 (Internal IP: 172.17.0.40)
Sys4 (Backend 3) : 10.1.75.79:2240 (Internal IP: 172.17.0.41)
"""
import os
import sys
import time
...
SYS1_PORT = 2237 # Load Balancer / Server
SYS2_PORT = 2238 # Backend 1
SYS3_PORT = 2239 # Backend 2
SYS4_PORT = 2240 # Backend 3
...
def kill_existing_processes():
    print("\n🧹 Cleaning up any existing backend / LB processes across all 4 systems...")
    for p, name in [(SYS1_PORT, "Sys1"), (SYS2_PORT, "Sys2"), (SYS3_PORT, "Sys3"), (SYS4_PORT, "Sys4")]:
        try:
            ...
            client.close()
            print(f"  ✅ Cleared processes on {name} (Port {p})")
        except Exception as e:
            print(f"  ⚠️ Could not cleanup {name}: {e}")
...
def run_distributed_pipeline():
    ...
    try:
        # 1. Start Backends on Sys2, Sys3, Sys4
        ...
        # 2. Verify independent reachability from Sys1
        ...
        # 3. Experiment 1: Single Backend (Sys2)
        ...
        # Stop single backend LB
        sys1_client.exec_command("pkill -9 -f balancer.py || true")
        time.sleep(1.0)

        # 4. Experiment 2: Three Backends (Sys2, Sys3, Sys4)
        ...
        # Run Workload for Experiment 2 (Identical Workload)
        print("\n[Step 5b/6] Executing Experiment 2 Workload on Sys1 (Identical Workload)...")
        stdin, stdout, stderr = sys1_client.exec_command(
        ...
        # 5. Generate Comparison Report on Sys1
        print("\n[Step 6/6] Generating Comparison Report on Sys1...")
        stdin, stdout, stderr = sys1_client.exec_command(
        ...
        print("\n📥 Fetching experiment results from Sys1 to local workspace...")
